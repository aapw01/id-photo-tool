...
| 2026-05-12 | M9 红边/平顶修复走 alpha-aware matte 前置去污染                      | 继续调阈值 / 换更快模型 / 立即引入 RMBG/BiRefNet                                                                          | 根因是 canvas `destination-in` 后外圈透明像素已丢失原红背景 RGB，旧去污染无法估计 spill；同时高端 alpha 硬饱和会削平头顶软边。本轮改为在原图 RGB 仍可读时应用 alpha matte + 反混背景色，并取消高端硬 clip。V1.1 可评估高质量模式：BRIA RMBG/BiRefNet 服务端或 WebGPU 可选模型。 |
...
| 2026-05-12 | 0.8.4 | M9 核心抠图质量二次修复：定位用户截图里的红/粉 halo 与头顶平边不是“模型速度”问题，而是后处理顺序和 alpha 曲线问题。`extractForeground` 改为在原始照片 RGB 仍存在时读取 mask alpha，先估计近边旧背景色并反混 soft-edge RGB，再写入前景 ImageData；`refineAlpha` 保留低端 cutoff 但取消高端硬饱和，避免把头顶细发过渡压成 255 平面。新增回归测试覆盖红背景 contamination、顶部高 alpha 连续性、透明/不透明像素无副作用；V1.1 路线记录可评估 RMBG/BiRefNet 高质量模式（浏览器可选或服务端）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
